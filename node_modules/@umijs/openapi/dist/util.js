...
const writeFile = (folderPath, fileName, content) => {
    ...
    (0, exports.mkdir)(path_1.default.dirname(filePath));
...
const getTagName = (name) => {
    ...
    if (result.length === 5) {
        return result[2] + (0, lodash_1.upperFirst)(result[3]);
    }
    return name;
};
...
const formatApiInfo = (apiInfo) => {
    ...
    apiInfo.schema.tags = apiInfo.schema.tags.map((item) => {
        return Object.assign(Object.assign({}, item), { name: (0, exports.getTagName)(item.name) });
    });
    for (const child_path in apiInfo.schema.paths) {
        apiInfo.schema.paths[child_path].post.tags = apiInfo.schema.paths[child_path].post.tags.map((tag) => (0, exports.getTagName)(tag));
    }
    return apiInfo;
};
...
function formatParamsForYFH(params, paramsObject = {}) {
    Object.keys(params).forEach((name) => {
        ...
        if (nameListLength === 1) {
            ...
            // key.child_key
            const key_child_key = (0, lodash_1.camelCase)(nameList[1]);
            ...
            if (nameList[nameListLength - 2] === 'n' || nameList[nameListLength - 2] === 'm') {
                const child_key = (0, lodash_1.camelCase)(nameList.pop());
                nameList.pop();
                key = nameList.join('.');
                ...
                if (child_key === 'n' || child_key === 'm') {
                    ...
                    if (nameList[nameList.length - 2] === 'n' || nameList[nameList.length - 2] === 'm') {
                        const child_child_key = (0, lodash_1.camelCase)(nameList.pop());
                        nameList.pop();
                        key = nameList.join('.');
        ...
        paramsObject[key].name = (0, lodash_1.camelCase)(key);
    });
